...
library(dplyr)
library(tidyverse)
# 大專院校校務資訊公開平臺 學12-1.新生註冊率-以「系(所)」統計
# https://udb.moe.edu.tw/DetailReportList/%E5%AD%B8%E7%94%9F%E9%A1%9E/StatDepartmentRegistrationRate/Index
R.Version()
rm(list=ls()) #刪除所有變數
filename='12-1應用外語系新生註冊率統計.csv'
...
tail(z)
write.csv(y[,c(5,6)],"schoolname.csv",row.names = F,fileEncoding = 'utf8')
z %>% arrange(新生註冊率)
z %>% arrange(-新生註冊率)
